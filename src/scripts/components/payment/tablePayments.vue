    ...
    <div v-if="!loading">
      <div class="row" id="tableRow">
        ...
        <b-table striped hover :items="paymentsData()" :fields="fields" :current-page="currentPage" :per-page="perPage" :filter="filter">
            ...
            <template slot="occupation" scope="item">
           <div v-if="item.value">  <!-- CONDITION TO VERIFY IF VALUE IS NULL -->
            {{ item.value.customer.firstName + " " + item.value.customer.lastName }}
            </div>
            <div v-else>
             Empty
            </div>
          ...
          <template slot="employee" scope="item">
           <div v-if="item.value">   <!-- CONDITION TO VERIFY IF VALUE IS NULL -->
            {{ item.value.employee.firstName + " " + item.value.employee.lastName }}
            </div>
            <div v-else>
             Empty
            </div>
          ...
          <template slot="paymentDay" scope="item">
           <div v-if="item.value">   <!-- CONDITION TO VERIFY IF VALUE IS NULL -->
            {{ item.value }}
            </div>
            <div v-else>
         ...
         <template slot="paid" scope="item">
          ...
          <template slot="consultButton" scope="item">
            <b-button variant="primary" size="sm" :to="{name:'consultationPayment', params: { payment: item.item }}">
              <!-- BUTTON TO CONSULT PAYMENT-->
              Consult
            </b-button>
